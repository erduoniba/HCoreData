...
+ (NSArray *)getItemsCount:(NSInteger)itemCount andStartIndex:(NSInteger)startIndex andType:(QUESTION_TYPE)questionType{
    ...
    NSArray *allItems = [self itemsWithPredicate:[NSPredicate predicateWithFormat:@"qo_type == %d", questionType]];
    
    if (itemCount + startIndex >= allItems.count) {
        itemCount = allItems.count - startIndex;
    }
    if (itemCount < 0) {
        itemCount = 0;
    }
    NSIndexSet *indexSet = [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(startIndex, itemCount)];
    return [allItems objectsAtIndexes:indexSet];
}
